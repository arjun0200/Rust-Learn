...
fn read_and_write(args: &Arguments){
    let data = match fs::read_to_string(&args.input_file){
        Ok(f) => f,
        Err(e) =>{
            eprintln!("{} failed to read file from {}: {:?}", "Error".red().bold(), args.input_file, e);
            std::process::exit(1);
        }
    };
    ...
    let replace_data = match replace(&args.pattern, &args.replace, &data){
        Ok(d) => d,
        Err(e) =>{
            eprintln!("{} failed to replace txt : {:?}", "Error".red(), e);
            std::process::exit(1);
        }
    };
    match fs::write(&args.output_file, &replace_data){
        Ok(_) => {},
        Err(e) =>{
            eprintln!("{} failed to read file from {}: {:?}", "Error".red().bold(), args.output_file, e);
            std::process::exit(1);
        }
    }
}
