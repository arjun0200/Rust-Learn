...
pub fn longest_common_subsequence(a: &str, b: &str) -> String{
    let a: Vec<char> = a.chars().collect();  //store all character into a vector
    ...
    for (i, mi) in a.iter().enumerate(){
        for (j, mj) in b.iter().enumerate(){
            //if mi == mj, there is a common char
...
mod tests{
    ...
    fn test_maxsubarray(){
        ...
        let array2 = vec![-4,3,-2,5,-3]; // 3 + -2 + 5 = 6
        assert_eq!(max_subarray(&array2), 6);
    }
}